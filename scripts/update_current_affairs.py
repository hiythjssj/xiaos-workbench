...
SOURCES=[
 ...
 {'name':'中国政府网·要闻','url':'https://www.gov.cn/yaowen/','base':'https://www.gov.cn/'},
 {'name':'新华网·时政','url':'https://www.news.cn/politics/','base':'https://www.news.cn/'},
 {'name':'新华网·时政联播','url':'https://www.news.cn/politics/szlb/index.html','base':'https://www.news.cn/'}]
HIGH=['习近平','中共中央','国务院','全国人大','全国政协','政策','条例','意见','办法','规划','会议','改革','发展','经济','就业','教育','科技','人工智能','创新','乡村振兴','民生','社会保障','养老','医疗','生态','绿色','法治','外交','国防','航天','新质生产力','十五五','统一大市场','消费','能源','交通运输','公共卫生','普法','工业和信息化部','国家发展改革委','教育部','人力资源社会保障部']
...
def classify(t):
 ...
 if any(k in t for k in ['法治','条例','办法','政策','人大','普法','行政复议']):return '政策法治'
...
def extract(src):
 ...
 for m in re.finditer(r'<a\b[^>]*href=["\']([^"\']+)["\'][^>]*>(.*?)</a>',page,re.I|re.S):
  ...
  cat=classify(title);out.append({'id':hashlib.sha1(url.encode()).hexdigest()[:12],'date':d.isoformat(),'category':cat,'source':src['name'],'title':title,'summary':'权威来源近期发布。建议结合原文核对政策表述、重要数字和发布主体。','exam_point':exam(cat),'essay_material':essay(cat),'score':score(title),'url':url})
 return out
...
def main():
 ...
 for s in SOURCES:
  try:
   got=extract(s);items+=got;stats[s['name']]=len(got)
 ...
 data={'updated_at':datetime.datetime.now(datetime.timezone.utc).isoformat(),'sources':[s['name'] for s in SOURCES],'source_stats':stats,'filter':'近14天 + 公考价值筛选','items':items}
